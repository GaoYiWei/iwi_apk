<template>
    <div>
        <vxe-form :data="formData" :rules="formRules" ref="borrowForm">
            <vxe-form-item :span="20">
                <template #default>
                    ...
                    <vxe-button @click="deliveryEvent()" :disabled="formData.audited&&!formData.delivered?false:true">借出</vxe-button>
                    <vxe-button @click="receiveEvent()" :disabled="formData.delivered&&!formData.received?false:true">收回</vxe-button>
            ...
            <vxe-form-item :span="4">
                <template #default>
                    <vxe-input v-model="searchVal" placeholder="请输入单号" type="search" clearable @keydown="enterSearch($event)" @search-click="searchEvent()"></vxe-input>
                </template>
            </vxe-form-item>
            ...
            <vxe-form-item title="用途" field="usedfor" :item-render="{}">
                <template #default="{ data }">
                    <vxe-input v-model="data.usedfor"></vxe-input>
                </template>
            </vxe-form-item>
            <vxe-form-item title="归还时间" field="prebackat"  :item-render="{}">
                <template #default="{ data }">
                    <vxe-input v-model="data.prebackat" type="date"></vxe-input>
                </template>
            </vxe-form-item>
        ...
        <vxe-form :data="formData">
            ...
            <vxe-form-item field="delivered" title="借出人" :span="4" :item-render="{}">
                <template #default="{ data }">
                    <vxe-input v-model="data.delivered" readonly></vxe-input>
                </template>
            </vxe-form-item>
            <vxe-form-item field="deliveredat" title="借出时间" :span="4" :item-render="{}">
                <template #default="{ data }">
                    <vxe-input v-model="data.deliveredat" readonly></vxe-input>
                </template>
            </vxe-form-item>
            <vxe-form-item field="received" title="入库人" :span="4" :item-render="{}">
                <template #default="{ data }">
                    <vxe-input v-model="data.received" readonly></vxe-input>
                </template>
            </vxe-form-item>
            <vxe-form-item field="receivedat" title="入库时间" :span="4" :item-render="{}">
                <template #default="{ data }">
                    <vxe-input v-model="data.receivedat" readonly></vxe-input>
                </template>
            </vxe-form-item>
        </vxe-form>
    </div>
    ...
    methods : {
        printEvent() {
            ...
            QRCode.toDataURL(this.formData.id).then(url => {
                const printTmpl = `
                    <table><tbody><tr><td>
                        <div class="header">借用单</div>
                        <img class="qrcode" src="`+url+`"/>
                        <div>
                            <div class="fill-row">
                                <span class="fill-title">单号：</span>
                                <span class="fill-value">`+this.formData.id+`</span>
                                <span class="fill-title">用途：</span>
                                <span class="fill-value">`+this.formData.usedfor+`</span>
                                <span class="fill-title">归还时间：</span>
                                <span class="fill-value">`+this.formData.prebackat+`</span>
        ...
        searchEvent() {
                ...
                this.$axios({
                    ...
                    if(res.data['borrow_m'].length==0) {
                        this.$message({ message: '没有找到记录', type: 'warning' })
                        return
                    }
                    this.submitLoading = false
                    this.formData = res.data['borrow_m'][0]
                    this.tableData = res.data['borrow_c']
        ...
        deliveryEvent() {
            var delivered=this.$store.state.user.name, deliveredat=new Date().toLocaleString('chinese', { hour12: false })
            this.submitLoading = true
            let data = { w: { id: this.formData.id }, v: { delivered: delivered, deliveredat: deliveredat } }
            this.$axios({
                method: 'PATCH',
                url: '/api/borrow',
                params: data
        ...
        receiveEvent() {
            var received=this.$store.state.user.name, receivedat=new Date().toLocaleString('chinese', { hour12: false })
            this.submitLoading = true
            let data = { w: { id: this.formData.id }, v: { received: received, receivedat: receivedat } }
            this.$axios({
                method: 'PATCH',
                url: '/api/borrow',
                params: data
        ...
        getData() {
            ...
            if(t.length==0) {
                this.$message({ message: '未填写借用明细', type: 'error' })
                return
            }
            res.push(t)
            return res
